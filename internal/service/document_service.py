...
class DocumentService(BaseService):
    ...
    def create_documents(
        ...
        if dataset is None or (
            account is not None and dataset.account_id != account.id
        ):
    ...
    def get_documents_status(self, dataset_id: UUID, batch: str, account: Account):
    ...
    def get_latest_document_position(self, dataset_id: UUID):
        """根据传递的知识库id获取最新文档位置"""
        document = (
            self.db.session.query(Document)
            .filter(
